...
void StackTester::testSetCapacity() {
	...
	// increase capacity in the pre-existing capacity
	Stack<unsigned> s1(3);
	s1.push(11);
	s1.push(22);
	s1.setCapacity(5);
	...
	// decrease capacity in the pre-existing capacity
	Stack<unsigned> s2(3);
	s2.push(11);
	s2.push(22);
	s2.push(33);
	try {
			s2.setCapacity(2);
			cerr << "\ns4.setCapacity is less than the Size\n";
			exit(1);
		} catch (const StackException& se) {
			cout << " 2 " << flush;
		}

	// Check that capacity doesn't change
	Stack<unsigned> s3(5);
	assert(s3.getCapacity() == 5);
	s3.setCapacity(5);
	assert(s3.getCapacity() == 5);
	cout << " 3 " << flush;
