...
#pragma omp parallel
{
		...
		for (int y = start; y < stop; ++y) {
			...
			for (int x = START_X; x < STOP_X; ++x) {
				...
				int invertedB = 255 - pixelColor.B;
				ColorInt invertedColor(invertedR, invertedG, invertedB);
				// draw the flip vertical at the same spot in canvas3
...
/*flipHorizontal function calls
 * original color and flips the
 * image horizontally using x
 * and y coordinates; two images
 * are now in chiral position.
 * Edited by Hansol Kim (hk94)
 */
 ...
 #pragma omp parallel for
	for (int y = START_Y; y < STOP_Y; ++y) {          // bottom to top
		...
		for (int x = START_X; x < STOP_X; ++x) {      // left to right
			// read the pixel at canvas0[row][col]
			ColorInt pixelColor = background0->getPixel(x, y);
			ColorInt Color(pixelColor.R +100, pixelColor.G +100, pixelColor.B +100);
			// draw the inverse at the same spot in canvas1
 ...
 * Edited by: Hansol Kim (hk94)
 */
void ImageConverter::blackNwhite()  {
	// record starting time
	double startTime = omp_get_wtime();
	...
	Background * background0 = myCanvas0.getBackground();
	Background * background5 = canvas5.getBackground();
  ...
  #pragma omp parallel for
	for (int y = START_Y; y < STOP_Y; ++y) {          // bottom to top
		...
		for (int x = START_X; x < STOP_X; ++x) {      // left to right
			// read the pixel at canvas0[row][col]
			ColorInt pixelColor = background0->getPixel(x, y);
			ColorInt Color((pixelColor.R + pixelColor.G + pixelColor.B) / 3);
			// draw the flip horizontal at the same spot in canvas1
			background5->drawPixel(x, y, Color);
		} // inner for
		// slow the processing to simulate a very large image
		canvas5.sleep();
	} // outer for
